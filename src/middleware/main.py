...
async def classify_sample(
    sample_info: ClassificationSampleInfo = Body(...),
...
def get_report(
    test_group_id: str = Query(
        ..., title="test group id", example="initial_dataset", description="test group id"
    )
) -> str:
    path_to_save = (
        "/reports/"
...
def get_classification_result(
    sample_info: ClassificationSampleInfo, classification_results: str
) -> tp.Tuple[bool, int]:
...
def post_to_queue_classify(sample_info: ClassificationSampleInfo, classified_class_id: int) -> None:
    ...
    channel.basic_publish(
        exchange=EXCHANGE_NAME, routing_key=CLASSIFICATION_RECORDER_QUEUE_NAME, body=body.json()
    )


def post_to_queue_report(test_group_id: str, path_to_save: str):
    body = BuildReportStatisticsHandler(test_group_id=test_group_id, save_path=path_to_save)
    channel.basic_publish(
        exchange=EXCHANGE_NAME, routing_key=CLASSIFICATION_RECORDER_QUEUE_NAME, body=body.json()
    )
